...
public class UIScrollView : MonoBehaviour
{
	...
	public float scrollWheelFactor = 0.6f;

	/// <summary>
	/// How much momentum gets applied when the press is released after dragging.
	/// </summary>

	public float momentumAmount = 60f;
	...
	public virtual Bounds bounds
	{
		get
		{
			if (!mCalculatedBounds && !isSetBounds)
			{
	...
	public bool RestrictWithinBounds (bool instant, bool horizontal, bool vertical)
	{
		...
		if (constraint.sqrMagnitude > 1f)
		{
			...
			{
//				Debug.Log(" ++++++++++++++ Spring Begin constraint " + constraint);
			...
			{
//				Debug.Log(" ++++++++++++++ MoveRelative constraint " + constraint);
	...
	/// Move the scroll view by the specified amount.
	/// </summary>
	public bool RestritMove = false; //by小鸡 限制复用item拉出窗口
	public delegate void MoveEvent();
	public MoveEvent moveevent;
	public void MoveAbsolute (Vector3 absolute)
	{
		if(RestritMove){
			if(absolute.y > 2.5 || absolute.y < -2.5){
				return;
			}
			if(null != moveevent){
				moveevent();
