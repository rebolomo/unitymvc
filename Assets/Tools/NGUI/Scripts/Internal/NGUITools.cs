...
static public class NGUITools
{
    ...
    static public GameObject AddChild(GameObject parent, GameObject prefab)
    {
		...
		/*
		//	REBOL note, 淇濊瘉prefab鐨刼bj鏍戝舰缁撴瀯涓嬬殑鎵鏈塽irect閮藉凡缁忚繍琛岃繃Start鍑芥暟锛屽惁鍒欎細鍑虹幇start鍑芥暟鎵ц?鐨勬椂鍊欙紝鎵句笉鍒皃arent panel鐨勯棶棰樸伾		//	鍙傝冿細http://www.tasharen.com/forum/index.php?topic=7210.0
		//go is the instantiated prefab
		foreach(UIRect rect in go.GetComponentsInChildren<UIRect>())
		{
			rect.enabled = false;
			rect.enabled = true;
		}
        ...
        if (obj != null)
        {
            if (Application.isPlaying)
            {
                if (obj is GameObject)
                {
					...
					//	REBOL note, 浣垮緱destroy鐨刧o涓嶅弬涓巙pdate
					//	鍙傝冿細http://7dot9.com/2015/12/15/ui-gameobject-layer-reset-by-ngui/
					SetActive (go, false);
    ...
    /// <summary>
    /// 鏌ユ壘鏌愯矾寰勭殑GameObject 鐨冞Component T 锛屽?鏋滃瓙GameObject瀛樺湪锛屼竴瀹氫細杩斿洖 UIWidgetContainer 
    /// Create By D.S.Qiu
    /// </summary>
    /// <typeparam name="T">Component type</typeparam>
    /// <param name="parent">鏌ユ壘鐨勬牴鑺傜偣</param>
    ...
    /// <summary>
    /// 杩斿洖 active 瀛怗ameObject Component T 鐨勪釜鏁?    /// </summary>
    /// <returns>The children count.</returns>
    /// <param name="includeInActive">If set to <c>true</c> include in active.</param>
    public static int GetChildrenCount<T>(GameObject go, bool includeInActive = false) where T : Component
    {
        Transform t = go.transform;
        int count = 0;
        Transform child;
        for (int i = 0, imax = t.childCount; i < imax; ++i)
        {
            ...
            if (t.GetComponent<T>() == null)
                continue;
            if (!includeInActive && GetActive(child.gameObject))
            {
                count++;
            }
            else
    ...
    /// <summary>
    /// 杩斿洖 active 瀛怗ameObject鐨勪釜鏁?    /// </summary>
    /// <returns>The children count.</returns>
    /// <param name="includeInActive">If set to <c>true</c> include in active.</param>
    public static int GetActiveChildrenCount(GameObject go)
    {
        Transform t = go.transform;
        int count = 0;
        Transform child;
    ...
    /// <summary>
    /// 鑾峰彇鎸囧畾娓告垙鐗╀綋鐨勬墍鏈夌洿鎺ュ瓙娓告垙鐗╀綋
    /// </summary>
    /// <param name="gameObject">鐖舵父鎴忕墿浣掻/param>
    /// <returns></returns>
    ...
    /// <summary>
    /// 娣诲姞瀛怗ameObject骞舵坊鍔燙omponet T
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="parent"></param>
    /// <param name="name"></param>
    /// <returns></returns>
    public static T AddChild<T>(GameObject parent, string name) where T : Component
    {
        GameObject child = new GameObject(name);
        T t = child.AddComponent<T>();
    ...
    //		
    //		if (null != btn.background)
    //		{
    //			btn.background.color = enabled ? Color.white : Color.gray;
    //		}
    //	}
    /// <summary>
    /// 璁剧疆褰撳墠灏勭嚎纰版挒鐐逛负localPosition
    /// </summary>
    /// <param name="tr">Tr.</param>
    public static void SetLocalPositionToRayHit(Transform tr)
    {
        Vector3 ve = UICamera.lastHit.point;
        tr.position = ve;
        //tr.localPosition = tr.InverseTransformPoint(ve);
    }
    /// <summary>
    /// NGUI 鍩烘湰缁勪欢 SetAcitve 鍑芥暟鎵╁睍
    /// </summary>
    /// <param name="rect"></param>
    /// <param name="state"></param>
    public static void SetActive(this UIRect rect, bool state)
    {
        if (rect.cachedGameObject.activeInHierarchy != state)
            rect.cachedGameObject.SetActive(state);
    }
}
